...
export default {
    ...
    methods:{
        keuze(event){
            ...
            if(event.target.className.baseVal.trim() == 'man'){
                this.geslacht = 'man'
                this.changeIcon('man')
            }else{
                this.geslacht = 'vrouw'
        ...
        changeIcon(value){
            this.removeClasses('path', 'changeFill')
            const keuzes = this.$el.querySelectorAll(`.${value}`)
            keuzes.forEach((keuze)=>{
                keuze.classList.add('changeFill')
            })
        },
